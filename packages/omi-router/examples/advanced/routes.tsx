...
const Layout = ({ children }: { children: VNode | VNode[] }) => (
  <>
    <h1>Head Navigation</h1>
    <div class="flex">
      <side-nav class="block w-60"></side-nav>
      <div class="flex-1 ml-10 mr-10 w-0 pt-10">{children}</div>
    </div>
  </>
)
...
export const routes = [
  {
    ...
    render() {
      return (
        <>
          <h1>Home</h1>
          <a href="#/user/1/profile">User1 Profile </a>
          <br />
          <a href="#/user/2/posts">User2 Posts </a>
          <br />
  ...
  },
  {
    ...
    render(router: Router) {
      return (
        <o-suspense imports={[import('./components/user-info')]}>
          ...
          <user-info>
            <o-suspense
              imports={[import('./components/user-profile')]}
              data={async () => {
  ...
  },
  {
    ...
    render(router: Router) {
      return (
        <o-suspense
          imports={[import('./components/user-info'), import('./components/user-posts')]}
          data={async () => {
