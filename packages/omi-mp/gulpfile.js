...
const components = series(copy, () => {
  const src = isWxComponent
    ? 'src/mp/components/**/*.js'
    : 'src/mp/components/*/*.js' // 判断是否微信组件

  return gulp
    .src(src)
    .pipe(
      tap(file => {
        let dir = isWxComponent
        ...
        try {
          if (path.basename(file.path) == name + '.js') {
            ...
            let componentWxss = isWxComponent
              ? fs.readFileSync('src/mp/component.wxss', 'utf8')
              : ''
            ...
            const helperPath =
              isWxComponent && preName === 'components'
                ? "'../../utils/helper'"
                : "'../../../utils/helper'"
  ...
  import { setData, fixProps, helpInputEvent } from ${helperPath}
    `
              ),
              Buffer.from(
  ...
  function css() {
    return '${
                isWxComponent ? componentWxss.replace(/[\r\n]/g, '') : ''
                }' + rpx(componentCss)
  }
...
function fileComponentDom(filePth) {
  let filePathArray = filePth
    .replace(__dirname, '')
    .replace('.js', '')
    .split(os.type().includes('Windows') ? '\\' : '/')
  filePathArray.shift()
...
function replaceComponentOnPath(tag, str, tagName) {
  ...
  return str.replace(reg, (match, $1, $2) => {
    if ($1) {
      return '<' + tagName
    }
    return '</' + tagName + '>'
