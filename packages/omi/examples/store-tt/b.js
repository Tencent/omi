(function () {
  ...
  store.undo = function () {
  	...
  	store.reset();
  	store.done.forEach(function (action) {
  		store._commit(action);
  	});
  };

  store._commit = function (action, record) {
  	if (record) store.done.push(action);
  	store.exec(action);
  };

  store.commit = function (action) {
  	store.undone.length = 0;
  	store.done.push(action);
  	store.exec(action);
  };

  store.exec = function (action) {
  	switch (action) {
  		case 'add':
  			store.add();
  			break;
  ...
  define('my-counter', function (_) {
  	return Omi.h(
  		...
  		Omi.h(
  			'button',
  			{ disabled: !_.store.done.length, onClick: _.store.undo },
  			'undo'
  		),
  		Omi.h(
  			'button',
  			{ disabled: !_.store.undone.length, onClick: _.store.redo },
  			'redo'
  		),
  		Omi.h('br', null),
  		Omi.h(
  ...
  render(Omi.h('my-counter', null), 'body', store);
