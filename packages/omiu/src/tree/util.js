...
export function toArray(children) {
  var ret = [];
  if (Object.prototype.toString.call(children) !== '[object Array]') warning(false, 'this only accept Array.');
...
export function isTreeNode(node) {
  return node && node.nodeName === 'tree-node';
}
...
export function convertTreeToEntities(treeNodes, { initWrapper, processEntity, onProcessFinished } = {}) {
  ...
  traverseTreeNodes(treeNodes, (item) => {
    ...
    const parentEntities = posEntities[parentPos];
    // if (entity.parent) {
    if (parentEntities) {
      entity.parent = parentEntities.key
      // entity.parent.children = entity.parent.children || [];
      parentEntities.children = parentEntities.children || [];
      // entity.parent.children.push(entity);
      parentEntities.children.push(key);
    }
...
export function conductCheck(keyList, isCheck, keyEntities, checkStatus = {}) {
  ...
  function conductUp(key) {
    ...
    (children || []).map(key => keyEntities[key])
    ...
    if (parent && keyEntities[parent]) {
      conductUp(keyEntities[parent].key);
    }
  }
  ...
  function conductDown(key) {
    ...
    (children || []).map(key => keyEntities[key]).forEach((child) => {
