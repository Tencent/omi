...
define('o-date-picker', class extends WeElement {
  ...
  install() {
    ...
    this.nextMonthDate = getNextMonth(this.now)
  ...
  getDay(y, x) {
    ...
    if (y === 0) {
      if (x < this.begin) {
        dateStr = this.preYear + '-' + this.preMonth + '-' + (this.preCount - this.begin + x + 1)
      ...
      if (temp <= this.count) {
        dateStr = this.year + '-' + this.month + '-' + temp
        if (temp === this.day) {
        ...
        dateStr = this.nextYear + '-' + this.nextMonth + '-' + (temp - this.count)
