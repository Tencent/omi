...
/***/ function(module, exports, __webpack_require__) {
	...
	var ES6TodoApp = _nuclear2.default.create({
	...
	;(function (root, factory) {
	    ...
	    Nuclear._mixObj = function (obj) {
	        ...
	        obj._mixNode = function () {
	            var nodes = this.node.querySelectorAll('*[nc-id]'),
	                len = nodes.length;
	            if (len > 0) {
	                var i = 0;
	                for (; i < len; i++) {
	            ...
	            var cNodes = this.node.querySelectorAll('*[nc-class]'),
	                cLen = cNodes.length;
	            if (cLen > 0) {
	                var j = 0;
	                for (; j < cLen; j++) {
	                ...
	                while (context) {
	                    if (name.indexOf('.') > 0) {
	                        ...
	                        while (value != null && index < names.length) {
	                            if (index === names.length - 1 && value != null) lookupHit = (typeof value === 'undefined' ? 'undefined' : _typeof(value)) === 'object' && value.hasOwnProperty(names[index]);
	                            value = value[names[index++]];
	                        }
	                    } else if (context.view != null && _typeof(context.view) === 'object') {
	    ...
	    ;(function () {
	        ...
	        observe.twoWay = function (objA, aProp, objB, bProp) {
	            if (_typeof(objA[aProp]) === "object" && _typeof(objB[bProp]) === "object") {
